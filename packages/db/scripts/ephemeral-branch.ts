 ...
 *
 * Usage:
 *   tsx scripts/ephemeral-branch.ts create   # Create branch and seed
 *   tsx scripts/ephemeral-branch.ts delete   # Delete branch and cleanup
 *
 * Prerequisites:
...
async function createBranch() {
  console.log("🌿 Creating ephemeral Neon branch for testing...\n");
...
`;
  ...
  console.log(`   ✅ Wrote ${ENV_TEST_FILE}`);
  ...
  console.log("\n" + "─".repeat(60));
  console.log("✅ Ephemeral test branch is ready!");
...
async function deleteBranch(branchName?: string, projectId?: string) {
  console.log("🗑️  Cleaning up ephemeral Neon branch...\n");

  // Read from env file if not provided
  if (!branchName) {
    branchName = readBranchFromEnv() ?? undefined;
    if (!branchName) {
      console.log("   ⚠️  No ephemeral branch found to delete");
      return;
    }
  }
  ...
  try {
    ...
    console.log(`   ⚠️  Branch deletion returned: ${error}`);
  }

  // Clean up .env.test file
  if (existsSync(ENV_TEST_FILE)) {
    unlinkSync(ENV_TEST_FILE);
    console.log(`   ✅ Removed ${ENV_TEST_FILE}`);
  }
...
switch (command) {
  ...
  case "delete":
    ...
    break;
  default:
    console.log("Usage: tsx scripts/ephemeral-branch.ts <create|delete>");
    console.log("");
